...
import { createAgentLatticeChatTool, createAgentLatticeDiscoverTool, createAgentLatticeInfoTool } from './agent-lattice-bridge.js';
...
export async function buildToolRegistry(getConfig: () => LegionConfig, legionHome?: string): Promise<ToolDefinition[]> {
  ...
  // Agent Lattice bridge — chat delegation + tool discovery + agent info
  if (config?.agentLattice?.enabled) {
    tools.push(createAgentLatticeChatTool(getConfig));
    tools.push(createAgentLatticeDiscoverTool(getConfig));
    tools.push(createAgentLatticeInfoTool(getConfig));
  }
