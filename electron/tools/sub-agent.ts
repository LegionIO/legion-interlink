...
export function createSubAgentTool(
  ...
  return {
    ...
    execute: async (input: unknown, ctx: ToolExecutionContext): Promise<unknown> => {
      ...
      const maxDepth = config.tools.subAgents.maxDepth;
      if (currentDepth >= maxDepth) {
        return { isError: true, error: `Sub-agent depth limit reached (max: ${maxDepth}).` };
      }
      if (getActiveSubAgentCount() >= config.tools.subAgents.maxConcurrent) {
        return { isError: true, error: `Maximum concurrent sub-agents (${config.tools.subAgents.maxConcurrent}) reached.` };
      }

      const modelKey = model ?? config.tools.subAgents.defaultModel ?? null;
