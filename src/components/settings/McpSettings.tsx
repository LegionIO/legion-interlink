  ...
  return (
    <div className={`rounded-lg border p-3 space-y-2 ${!isEnabled ? 'opacity-50' : ''}`}>
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-mono font-semibold truncate">{highlightBrandText(server.name)}</span>
      ...
      <div className="text-[10px] text-muted-foreground font-mono truncate">
        {server.url ? highlightBrandText(server.url) : `${server.command ?? ''} ${(server.args ?? []).join(' ')}`.trim()}
      </div>
