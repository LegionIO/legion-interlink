...
/**
 * Single-line contentEditable input that renders "Legion" with the gradient.
 * Uses the same highlight/cursor logic as EditableTextarea but restricted to one line.
 */
export const EditableInput: FC<EditableInputProps> = ({
  ...
  const toHTML = useCallback((text: string): string => {
    ...
    if (!highlightBrand) return text;
    return text.replace(/legion/gi, (m) => `<span class="legion-gradient-text font-semibold">${m}</span>`);
  }, [highlightBrand, type]);
