...
/**
 * An editable div that looks/behaves like a textarea but renders "Legion"
 * with the gradient animation inline. Uses contentEditable with careful
 * cursor preservation to avoid the double-vision overlay problem.
 */
export const EditableTextarea: FC<EditableTextareaProps> = ({
  ...
  // Highlight "Legion" in the editor content
    ...
    return text
      ...
      .map((line) =>
        highlightBrand
          ? line.replace(
              /legion/gi,
              (match) => `<span class="legion-gradient-text font-semibold">${match}</span>`,
            )
          : line,
      )
      .join('<br>');
  ...
  const highlightLegion = useCallback(() => {
    const el = editorRef.current;
    if (!el) return;
  ...
  const handleInput = () => {
    ...
    if (nowEmpty) {
      ...
      requestAnimationFrame(highlightLegion);
    }
  };
